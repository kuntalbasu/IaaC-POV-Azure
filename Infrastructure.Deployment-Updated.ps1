...
# Not necesary that all veriables will be used.
    ...
    $InternalvNet1 = "vnet-internal-1"
    $InternalvNet2 = "vnet-internal-2"
...
#############################################################################################
    ...
    $VNet = New-AzVirtualNetwork -ResourceGroupName $resourceGroup -Location $location -Name $InternalvNet1 -AddressPrefix 192.168.1.0/24 -Subnet $subnetConfig1, $subnetConfig2
...
# Followings are 2 examples of GUI and Core Windows Versions
    ...
    $PriIP = 192.168.1.11
    $NIC = New-AzNetworkInterface -Name $NICName -ResourceGroupName $ResourceGroup -Location $Location -SubnetID $SubnetID2 -PublicIpAddressId $PriIP -NetworkSecurityGroupId $nsg.Id
